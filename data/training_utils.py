...
def chooseship(data, ship_name):
    ship_name = ship_name.rstrip()
    ship_name = ship_name.lstrip()
    return data["ship_df"][data["ship_df"]['Ship_Name'] == ship_name]['object'].iloc[0]
...
def refill_ports(data,ports):
    for i in ports['port_object'].to_list():
        i.demand = port_items(data["all_port_item"],i.name)
