...
class JaringanPelabuhan():
    ...
    def add_extraRoute(self,rute):
        self.extra_route = rute
    ...
    def get_full_path(self,distance,  rute = []):
        extra_route = []
        if len(rute) == 2:
            if tuple(rute) in self.extra_route.keys():
                extra_route = self.extra_route[tuple(rute)].copy()
            elif tuple([rute[1], rute[0]]) in self.extra_route.keys():
                extra_route = self.extra_route[tuple([rute[1], rute[0]])].copy()
                extra_route.reverse()
        ...
        for n, name in enumerate(rute):
            ...
            if extra_route == []:
                ...
                path_a = []
                if (awal,akhir) in self.extra_route.keys():
                    for i in self.extra_route[(awal,akhir)]:
                        path_a.append(i)
                elif (akhir,awal) in self.extra_route.keys():
                    path = self.extra_route[(akhir,awal)].copy()
                    path.reverse()              
                    for i in path:
                        path_a.append(i)
...
#            pel_bobot = ({name: sum([j["Bobot"] for j in self.lis_pelabuhan[name].barang]) for name in self.lis_pelabuhan})
    ...
    def loading_barang(self, kapal, pel_singgah, c):
        ...
        for n, brg in enumerate(self.lis_pelabuhan[pel_singgah].barang):
            new_barang = kapal.ambil_barang(brg, pel= self)
            if new_barang == None:
        ...
        for n, kode_barang in enumerate(self.lis_pelabuhan[pel_singgah].barang_transit):
            if kapal.kategori == "PR" and self.lis_pelabuhan[pel_singgah].barang_transit[kode_barang]["Tujuan Pelabuhan"] in self.pel_cuaca_tinggi:
                pass
            else:
                new_barang = kapal.ambil_barang(self.lis_pelabuhan[pel_singgah].barang_transit[kode_barang],mode = "Transit", pel= self)
                if new_barang == None:
                    delete_list.append(n)
                else:
            ...
            if hasil != {}:
                ...
                for i in kpl.barang:
                    for j in kpl.emergency_transit:
                        tujuan += kpl.emergency_transit[j]
                    ...
                    if kpl.kategori in ["PR"]:
                        for i in list_barang:
                            if type(i["Transit"]) == list:
                                ...
                                trans = i["Transit"]
                            if (trans in kpl.rute_name and i["Tujuan Pelabuhan"] not in kpl.rute_name):
                                 tujuan += [trans]
                            if i["Tujuan Pelabuhan"] in kpl.rute_name:
                                tujuan += [i["Tujuan Pelabuhan"]]
                        
                        for i in list_barang2:
                            if i["Tujuan Pelabuhan"] in kpl.rute_name:
                                tujuan += [i["Tujuan Pelabuhan"]]
                        ...
                        for i in list_barang2:
                            ...
                            if kpl.kategori == "TL":
                                if i["Tujuan Pelabuhan"] in self.pel_cuaca_tinggi:
                                    tujuan += [pel]

                for i in self.pel_cuaca_tinggi:
                    if self.lis_pelabuhan[i].barang and  kpl.kategori == "TL" :
                        tujuan += [i]
                if  kpl.kategori == "TL":
                    if list(set(kpl.rute_name).intersection(set(tujuan))) == [] and  (kpl.beban_angkut == 0) and (list(set(self.pel_cuaca_tinggi).intersection(set(tujuan))) == [] and kpl.kategori == "TL") and kpl.rute2["nama"].split(" - ")[1] == kpl.rute_name[0]:
    ...
    def unloading_barang(self,kapal, pel_singgah):
        ...
        for n, brg in enumerate(kapal.barang):
            if kapal.kategori == "PR":
                delete = False
                if brg["Tujuan Pelabuhan"] in kapal.emergency_transit.keys():
                    delete = True
                    if pel_singgah == kapal.emergency_transit[brg["Tujuan Pelabuhan"]]:
                ...
                if delete:
                    del kapal.emergency_transit[brg["Tujuan Pelabuhan"]]
...
class Kapal():
    def __init__(self,pelabuhan, nama, kategori, kapasitas, max_voyage, rute, speed, data, time_step = 0.1):
        self.extra_port_count = 0
        self.rute_name = rute
...
#        self.waktu_singgah_max = 5
    ...
    def update(self, pelabuhan):
        if self.started:
            ...
            self.started = False
            # print(self.nama, self.barang_kosong)
            # if self.barang_kosong:
            #     self.count = len(self.rute2["rute"])+1
            #     print(self.count)
            #     self.current_port = self.rute_name[0]
            
        elif (not self.is_singgah):
            ...
            if self.singgah <= self.waktu_singgah_max*60 or (self.barang_kosong) :
                ...
                if self.current_port == self.rute_name[0]:
                    n = len(self.jumlah_trayek) + 1
                    self.jumlah_trayek[n] = self.lama_perjalanan
                    self.lama_perjalanan = 0
                self.check_available_port(pelabuhan)
...
#              self.is_singgah = False
    ...
    def get_data(self):
        return {
                ...
                "Lama Perjalanan" : str(timedelta(minutes=self.lama_perjalanan))[:-3] + f" ({len(self.jumlah_trayek)} Voyage)", 
#                "Lokasi Sekarang": str(int(100*self.current_position/self.total_path))+"%",
    ...
    def ambil_barang(self,brg, mode = "No Transit", pel = None):
        ...
        if mode == "No Transit":
            ...
            elif (self.kategori == "TL" and barang["Asal Pelabuhan"] in pel.pel_cuaca_tinggi):
                if (self.kapasitas - self.beban_angkut) != 0 :
                    ambil = barang['Bobot'] - (self.kapasitas - self.beban_angkut)
                    if ambil > 0 :
                        barang2['Bobot'] = (self.kapasitas - self.beban_angkut)
            ...
            if barang["Tujuan Pelabuhan"] in self.rute_name:
                if (self.kapasitas - self.beban_angkut) != 0 :
                    ...
                    if ambil > 0 :
                        ...
                        self.barang.append(barang)
                        barang = None
            elif (self.kategori == "TL" and barang["Tujuan Pelabuhan"] in pel.pel_cuaca_tinggi):
                if (self.kapasitas - self.beban_angkut) != 0 :
                    ambil = barang['Bobot'] - (self.kapasitas - self.beban_angkut)
                    if ambil > 0 :
                        barang2['Bobot'] = (self.kapasitas - self.beban_angkut)
...
#        self.update_kapasitas()
    ...
    def check_available_port(self, pelabuhan):
        kosong = False
        idx = 0 
        if self.current_port not in self.rute_name:
            a = [i["Tujuan Pelabuhan"] for i in self.barang if i["Asal Pelabuhan"] == self.current_port]
            if a:
                next_port = a[0]
            else:
                d = None
                for n,i in enumerate(self.rute_name):
                    ...
                    if d is None:
                        ...
                        if d > l:
                            idx = n
                            d = l
                next_port = self.rute_name[(idx) % len(self.rute_name)]
                idx = (idx-1) % len(self.rute_name)
        else:
